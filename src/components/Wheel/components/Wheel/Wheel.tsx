...
import WheelCanvas, {
  WheelData,
} from '../WheelCanvas/WheelCanvas'
import { getRotationDegrees } from '../../../../helpers/Roulette/getRotationDegrees'
...
export const Wheel = ({
  ...
  const normalizedSpinDuration = Math.max(
    0.01,
    spinDuration
  )

  const startSpinningTime =
    START_SPINNING_TIME * normalizedSpinDuration
  const continueSpinningTime =
    CONTINUE_SPINNING_TIME * normalizedSpinDuration
  const stopSpinningTime =
    STOP_SPINNING_TIME * normalizedSpinDuration
  ...
  useEffect(() => {
    if (mustStartSpinning && !isCurrentlySpinning) {
      ...
      const finalRotationDegreesCalculated =
        getRotationDegrees(prizeNumber, data.length)
